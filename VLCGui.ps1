###################################################################
# VCF Lab Constructor beta v4.4.0 1/27/2022
...
if ($global:Ways -notmatch "expansion" -and $bringupAfterBuild) {
    $apiTokens = sddcTokenPairCreate -sddcMgrIP $sddcMgrIP -userName $ssoCredential -passWord $ssoAdminPassword
    ...
    if (![string]::IsNullOrEmpty($global:userOptions.extraHosts)) {
        ...
        $bulkHostPayload = Get-Content $($global:userOptions.extraHosts) | ConvertFrom-JSON | Select -ExpandProperty genVM | Select @{l='fqdn';e='name'} -First 3
