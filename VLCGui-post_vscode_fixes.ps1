###################################################################
# VCF Lab Constructor beta v4.4.0 2/9/2022
...
$host.ui.RawUI.WindowTitle = 'VCF Lab Constructor beta v4.4 - Process Window'
$welcomeText = @"
Welcome to:
...
Function cbConfigurator {
    ...
    $uplinkAddrs = $uplinkInfo | Select-Object -ExpandProperty gateway | ForEach-Object { "$_/$($($uplinkInfo | Select-Object -ExpandProperty subnet).Split('/')[1])" }
    $edgeCidrs = foreach ($edgeCidr in $edgeNeighbors) { $($edgeCidr.uplinkInterfaceIP | Select-Object -Unique).Split("/")[1] }
    ...
    if ($($userOptions.mgmtNetVlan) -ne 0) {
        ...
        $replaceNet += "echo VLAN=eth0.$($edgeNodeSpecs | Select-Object -ExpandProperty edgeTepVlan -Unique)`n"
    ...
    $replaceNet += "(`n"
    $replaceNet += "echo [NetDev]`n"
    $replaceNet += "echo Name=eth0.$($edgeNodeSpecs.edgeTepVlan | Select-Object -Unique)`n"
    $replaceNet += "echo Kind=vlan`n"
    $replaceNet += "echo [VLAN]`n"
    $replaceNet += "echo Id=$($edgeNodeSpecs.edgeTepVlan | Select-Object -Unique)`n"
    $replaceNet += ")>/etc/systemd/network/eth0.$($edgeNodeSpecs.edgeTepVlan | Select-Object -Unique).netdev`n"
    $replaceNet += "(`n"
    $replaceNet += "echo [Match]`n"
    $replaceNet += "echo Name=eth0.$($edgeNodeSpecs.edgeTepVlan | Select-Object -Unique)`n"
    $replaceNet += "echo [Network]`n"
    ...
    $replaceNet += ")>/etc/systemd/network/eth0.$($edgeNodeSpecs.edgeTepVlan | Select-Object -Unique).network`n"
    ...
    $replaceNet += "echo ExecStart=/sbin/ifconfig eth0.$($edgeNodeSpecs | Select-Object -ExpandProperty edgeTepVlan -Unique) mtu 8940 up`n"
    ...
    $replaceDNS += "echo recursive_acl = \`"$CloudBuilderIPSubnet/$CloudBuilderCIDR,$($avnNets[0]),$($avnNets[1]),$($tzEgress),$($tzIngress)\`"`n"
...
Function compileDNSRecords {
    ...
    #Additional Hosts
    if ($jsonLoc -ne "") {
        Get-Content -raw $jsonLoc | ConvertFrom-Json | Select-Object -ExpandProperty genVM | ForEach-Object -Process { $dnsFQDNs.Add($_.name, $_.mgmtIP) }
    }
    #vCenter
...
function sddcTokenRefresh ($sddcMgrIP, $apiTokens) {
    ...
    $tokenRefreshParms = @{
        ...
        Body        = "`"$($apiTokens.refreshToken | Select-Object -ExpandProperty id)`""
    }
...
$chkWldMgmt.location = new-object system.drawing.point(415, 375)
...
$lblchkWldMgmt.Width = 95
$lblchkWldMgmt.Height = 20
$lblchkWldMgmt.Enabled = $true
$lblchkWldMgmt.location = new-object system.drawing.point(436, 375)
...
$chkAVNs.location = new-object system.drawing.point(305, 375)
...
$lblchkAVNs.location = new-object system.drawing.point(321, 375)
...
#>                    
            ...
            logger "Deploy AVNs:"
            logger $chkAVNs.Checked
            logger "Deploy Edge Cluster" 
            logger $chkEC.Checked
            logger "Addtl Hosts choice"
            logger $comboBoxBuildOps.SelectedItem
            logger "bringup JSON:"
...
        logger "$msg $status $error[0]"
    ...
    logger "New HBA: $(Get-SCSIController -VM $VM_Name | Select-Object -ExpandProperty Type)"
    ####<New Debug Code>####
...
If (!$chkHostOnly.Checked) {
    ...
    if ($chkUseCBIso.Checked -eq $true) {
        ...
        If (Test-Path -Path $copyEsxiPath) {
            ...
            $vSphereISOLoc = $(Get-ChildItem -Path "$scriptDir\Temp\*.iso" | Select-Object FullName).FullName        
        }
    }
    else {
...
"@
    ...
    Invoke-VMScript -ScriptType Bash -Server $managementVCIP -GuestUser root -GuestPassword $($userOptions.masterPassword) -VM $sddcManagerVM -ScriptText "chmod 777 /home/vcf/DomainManagerConfig.bash;/home/vcf/DomainManagerConfig.bash"
    do {
    ...
    $apiUp = $false
    logger "Waiting for SDDC Manager API to be available..."
    do {
        try {
            $checkSDDCAPIParms = @{
...
if ($global:Ways -notmatch "expansion" -and $global:bringupAfterBuild) {
    $apiTokens = sddcTokenPairCreate -sddcMgrIP $sddcMgrIP -userName $ssoCredential -passWord $ssoAdminPassword
    logger "Obtaining API Token from SDDC Manager"

    if ($global:userOptions.deployEdgeCluster) {
    ...
    if ($global:userOptions.deployWldMgmt) {
        $failOut = $false
        ...
        $mgmtNetworkName = $($global:bringupOptions | Select-Object -ExpandProperty networkSpecs | Where-Object { $_.networkType -match "MANAGEMENT" } | Select-Object -ExpandProperty portGroupKey)
    ...
    if ($global:userOptions.deployAVNs) {
    ...
    if (![string]::IsNullOrEmpty($global:userOptions.extraHosts)) {
        ...
        logger "Loading Additional Host file selected - $($global:userOptions.extraHosts)"
        $bulkHostPayload = Get-Content $($global:userOptions.extraHosts) | ConvertFrom-JSON | Select-Object -ExpandProperty genVM | Select-Object @{l = 'fqdn'; e = 'name' } -First 3
